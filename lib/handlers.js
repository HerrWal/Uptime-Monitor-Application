...
handlers.index = (data, callback) => {
    ...
    if (data.method == 'get') {
        ...
        helpers.getTemplate('index', templateDate, (err, str) => {
            if (!err && str) {
                // Add the universal header and footer
                helpers.addUniversalTemplates(str, templateDate, (err, str) => {
                    if (!err && str) {
        ...
        _data.read('users', phone, (err, userData) => {
            if (!err && userData) {
                ...
                if (hashedPassword == userData.hashedPassword) {
                    ...
                    callback(400, {'Error' : 'Password did not match the specified user\'s stored passsword'});
                }
            } else {
                callback(400, {'Error' : 'Could not find the specified user'});
            }
        ...
        _data.read('tokens', token, (err, tokenData) => {
            if (!err && tokenData) {
                ...
                _data.read('users', userPhone, (err, userData) => {
                    if (!err && userData) {
                        const userChecks = typeof(userData.checks) == 'object' && userData.checks instanceof Array ? userData.checks : [];
                        // Verify that the user has lkess than the number of max-checks-per-user
...
handlers._checks.put = (data, callback) => {
    ...
    if (id) {
        // Check to make sure one or more optimal fields has been sent
        if (protocol || url || method || successCodes || timeoutSeconds) {
            // Lookup the check
            _data.read('checks', id, (err, checkData) => {
                if(!err && checkData) {
